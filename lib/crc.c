...
/* Slice-by-4 lookup tables*/
static const uint32_t crc32_sliceby4_table[][256] = {
  {
...
crc32_update_no_xor_slice_by_4 (uint32_t crc, const uint32_t *buf)
{
  ...
  return crc32_sliceby4_table[3][(crc >> 24) & 0xFF] ^ \
    ...
    crc32_sliceby4_table[1][(crc >> 8) & 0xFF] ^ \
    crc32_sliceby4_table[0][crc & 0xFF];
}
...
crc32_update_no_xor_slice_by_8 (uint32_t crc, const uint32_t *buf)
{
  ...
  crc = crc32_sliceby8_table[3][(crc >> 24) & 0xFF] ^ \
    ...
    crc32_sliceby8_table[1][(crc >> 8) & 0xFF] ^ \
    crc32_sliceby8_table[0][crc & 0xFF];

  return crc ^ crc32_update_no_xor_slice_by_4(0L, buf + 1);
}

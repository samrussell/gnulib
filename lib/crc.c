 ...
 */
uint32_t
crc32_update_no_xor_by_byte (uint32_t crc, const char *buf, size_t len)
{
  size_t n;
...
crc32_update_no_xor_slice_by_8 (uint32_t crc, const char *buf)
{
  ...
  crc =
    crc32_sliceby8_table[7][(local_buf >> 56) & 0xFF] ^ \
    crc32_sliceby8_table[6][(local_buf >> 48) & 0xFF] ^ \
    crc32_sliceby8_table[5][(local_buf >> 40) & 0xFF] ^ \
    crc32_sliceby8_table[4][(local_buf >> 32) & 0xFF] ^ \
    crc32_sliceby8_table[3][(local_buf >> 24) & 0xFF] ^ \
    crc32_sliceby8_table[2][(local_buf >> 16) & 0xFF] ^ \
    crc32_sliceby8_table[1][(local_buf >> 8) & 0xFF] ^ \
    crc32_sliceby8_table[0][local_buf & 0xFF];
...
crc32_update_no_xor_slice_by_4 (uint32_t crc, const char *buf)
{
  ...
  local_buf = local_buf ^ crc;
  crc =
    crc32_sliceby8_table[7][(local_buf >> 24) & 0xFF] ^ \
    crc32_sliceby8_table[6][(local_buf >> 16) & 0xFF] ^ \
    crc32_sliceby8_table[5][(local_buf >> 8) & 0xFF] ^ \
    crc32_sliceby8_table[4][local_buf & 0xFF];

  return crc;
}

uint32_t
crc32_update_no_xor_slice_by_2 (uint32_t crc, const char *buf)
{
  uint64_t local_buf;
  memcpy(&local_buf, buf, 2);
  local_buf = local_buf ^ crc;
  crc = (crc >> 16) ^ \
    crc32_sliceby8_table[7][(local_buf >> 8) & 0xFF] ^ \
    crc32_sliceby8_table[6][local_buf & 0xFF];

  return crc;
}

uint32_t
crc32_update_no_xor_slice_by_1 (uint32_t crc, const char *buf)
{
  uint64_t local_buf;
  memcpy(&local_buf, buf, 2);
  local_buf = local_buf ^ crc;
...
crc32_update_no_xor (uint32_t crc, const char *buf, size_t len)
{
  ...
  if (len - n >= 4) {
    crc = crc32_update_no_xor_slice_by_4(crc, buf + n);
    n += 4;
  }

  if (len - n >= 2) {
    crc = crc32_update_no_xor_slice_by_2(crc, buf + n);
    n += 2;
  }

  if (len - n >= 1){
    crc = crc32_update_no_xor_slice_by_1(crc, buf + n);
  }
